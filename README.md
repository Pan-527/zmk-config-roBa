...
- J を離した後もボールは惰性で回るので、その間は GESTURE_SETTLE レイヤー (8) が「J 押下中の最後のボール入力から 700ms」だけ自動で有効になり、同じく移動量を 0 にします。J を離した後は延長されないので、最大 700ms で通常のカーソル操作に戻ります。この時間は `&trackball_listener` 内の `&zip_temp_layer 8 700` の 2 つ目の引数で変更できます。
