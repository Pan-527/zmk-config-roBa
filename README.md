...
- J を押している間は 1 回しか発火しません。発火すると GESTURE_DONE レイヤー (8) が有効になってボールの動きを無視し、J を離すと解除されます。これは 1 回のフリックで 2 回発火したり、指を戻す動きで逆方向に発火したりするのを防ぐためです。
- 割り当ては `config/roBa.keymap` の `macros` 内 `gesture_right` / `gesture_left` / `gesture_up` / `gesture_down` で変更できます。
- 感度は `&trackball_gesture` の `tick` で調整します (小さいほど少ない動きで発火)。
