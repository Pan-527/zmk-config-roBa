...
左手のロータリーエンコーダーを回すとマウスホイールとしてスクロールします (デフォルトレイヤー)。以前の音量調整は FUNCTION レイヤー (TAB 長押し) に移しています。

- 方向が逆なら `config/roBa.keymap` の `encoder_msc` の `SCRL_DOWN` と `SCRL_UP` を入れ替えてください。
- 1 ノッチあたりの量は keymap 先頭の `ZMK_POINTING_DEFAULT_SCRL_VAL` (80) で調整します。
