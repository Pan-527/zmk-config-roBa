...
- 左右は J を押している間は何度でも発火します (右→左の往復ができます)。上下は同じ方向が連続で発火しません。Mission Control はトグルなので、1 回のフリックで 2 回発火すると開いてすぐ閉じてしまうためです。上に弾くと GESTURE_UP_DONE レイヤー (8) が上だけを無視し、下に弾くと GESTURE_DOWN_DONE レイヤー (9) が下だけを無視するので、「上で Mission Control を開いて下で閉じる」はできます。J を離すと解除されます。
