...
- J を離した後もボールは惰性で回るので、その間は GESTURE_SETTLE レイヤー (8) が「最後のボール入力から 800ms」だけ自動で有効になり、同じく移動量を 0 にします。この時間は `&trackball_listener` 内の `&zip_temp_layer 8 800` の 2 つ目の引数で変更できます。
