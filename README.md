...
- SCROLL レイヤーの 2 か所が素の Cmd になっているので、Cmd+ホイール (Figma やブラウザのズーム) ができます。K + N + ボールなら右手だけ (薬指 K + 人差し指 N + 親指ボール)、K + 左親指 Cmd + ボールなら両手を使います。J や L は左クリック / 右クリックのコンボと重なって誤爆するため、どのコンボにも入っていない N を使っています。
...
H と J を同時に押している間、中ボタン (ホイールクリック) を押しっぱなしにします。そのままボールを転がすと中ボタンドラッグになるので、Figma などでキャンバスを斜めにも自由に掴んで動かせます。人差し指 H + 中指 J を押さえたまま親指でボールを回します。

コンボなので長押しの待ち時間がなく、押した瞬間から効きます。直前 125ms 以内に別のキーを打っていた場合は発火しないので、タイピング中に誤って中クリックになることはありません。デフォルトレイヤーでのみ有効です。

### Figma での操作まとめ

| 操作 | やり方 |
| --- | --- |
| キャンバスを掴んで移動 | H+J を押しながらボール (斜めにも動かせる) |
| キャンバスをスクロール | K を押しながらボール (縦横どちらかに寄る) |
| ズーム | K + N を押しながらボール (右手だけ)、または K + 左親指 Cmd + ボール |
| 左クリック / ドラッグ | J+K を押しながらボール |
| 右クリック | K+L |
...
- 左右は J を押している間は何度でも発火します (右→左の往復ができます)。上下は同じ方向が連続で発火しません。Mission Control はトグルなので、1 回のフリックで 2 回発火すると開いてすぐ閉じてしまうためです。上に弾くと GESTURE_UP_DONE レイヤー (8) が上だけを無視し、下に弾くと GESTURE_DOWN_DONE レイヤー (9) が下だけを無視するので、「上で Mission Control を開いて下で閉じる」はできます。このレイヤーはボールが止まって 300ms 経つか J を離すと解除されるので、少し間を置けば同じ方向をもう一度弾けます (デスクトップ表示を下→下で開閉するなど)。
- 下は以前 App Exposé (Ctrl+↓) でしたが、App Exposé を開く瞬間に macOS の Dock がデッドロックして再起動が必要になる事象が起きたため、デスクトップを表示 (F11) に変更しています。
